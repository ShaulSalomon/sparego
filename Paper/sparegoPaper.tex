...
To test the performance of sParEGO two situational testing problems based on a toolkit for generating stochastic multiobjective test problems \cite{Salomon2016Toolkit} where proposed. Using this toolkit the WFG4 problem has been modified to allow the modification of the modality (number of peaks), ruggedness, squeeze and the polynomial order as the equation \ref{eq:WFG4modified} explains:
\begin{equation}
\begin{aligned}
	t1 &= \frac{|\vx-C|}{\floor{C-\vx}+C} \\
	t2 &= \brr{4 \times A +2}\times\pi\times\brr{0.5 - \frac{t1}{2}} \\
	t3 &= \brr{1-|t1|}^{2\times D} \\
	\vf\brr{\vx, A, B, C, D, E} &= \frac{1+ \cos{t2 \times t3} + B\times |t1|^E}{B+2}
\end{aligned}
...
The second test problem named P2 will be a second degree polynomial stochastic problem and with no modality, therefore $A = 0$, $B = 10$, $C = 0.45$, $D = 0$, and $E = 2$. The two test problems can be seen in Figure \ref{fig:TestProblems}.  
